...
public final class PollHandler {
	...
	private static final Command YAY_COMMAND = new Command("!yay") {
	...
	private static final Command NAY_COMMAND = new Command("!nay") {
			...
			while (!TwitchBot.killIssued()) {
				try {
					for (int i = 0; i < 4; i++) {
						...
						if (msLeft > 0) {
							TwitchBot.getIRCConnection().sendMessage(TwitchBot.DEFAULT_CHANNELS[0], TimeUnit.MILLISECONDS.toSeconds(msLeft)+" seconds left to cast your vote.");
						}
					}
				...
				// Poll has ended.
				TwitchBot.getIRCConnection().sendMessage(TwitchBot.DEFAULT_CHANNELS[0], "The poll has ended. Results:");
				...
				for (Command command : tempCommands) {
					CommandHandler.removeTempCommand(command); // Clear commands first, so they can't accidently vote in the small amount of time.
				}
				votedUsers.clear();
				votes.clear();
				tempCommands.clear();
				...
				synchronized (this) {
					try {
						POLL_MANAGER.wait(); // Waiting for notification of poll.
					} catch (InterruptedException e) {
					} 
				}
			}
		...
		if (!pollActive() && canStartPoll(u)) {
			...
			if (split.length > 1) { 
				...
				for (String command : split[1].split(" ")) {
					...
					if (command.length() > 0) {
					tempCommands.add(new Command("!"+command){
