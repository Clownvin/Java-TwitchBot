...
public class SendMessage extends Command {
	...
	if (args.length > 1) { // Must have at least two, first will
		...
		case "all":
		    ...
		    break;
		case "this":
		    user.getBot().getIrcConnection().sendMessage(user.getChannel(), message.replace(args[0] + " ", ""));
		    break;
		default:
		    user.sendWhisper(
			    args[0] + " is not a valid destination. Valid destinations are: this, all, #channel");
		}
	    }
	} else {
