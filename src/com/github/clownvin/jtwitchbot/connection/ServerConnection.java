...
public final class ServerConnection extends Thread {
	...
	public void run() {
		...
		try {
		    Thread.sleep(333); // About 90 messages every 30 seconds
				       // MAX.
		} catch (InterruptedException e) {
		    continue; // If it's interrupted, we can't risk getting
			      // banned by over stepping our rate.
		}
		if (messages.size() < 1) {
		    continue;
		}
	...
	sendCommand("USER", String.format("%s %s bla :%s", bot.getAccount().getUsername(), serverInfo.getServerIp(),
