...
public final class MessageHandler {
	...
	bot.getChannelManager().getChannel(message.channel).addMessage(message);
	...
	bot.getChannelManager().getChannel(message.channel)
		.addMessage(new Message(message.user, "Whisper", message.message));
	...
	if (bot.getChannelManager().getChannel(message.channel).isRegisteredOnly()
		&& !message.user.getUserData().isRegistered()) {
	...
	if (message.message.length() > 5
		&& message.user.getBot().getChannelManager().getChannel(message.user.getChannel()).getModerateOn()) {
